...
public class UserController {
    ...
    @GetMapping({"","/"}) public Page<UserResponse> list(@RequestParam(defaultValue="1") int page,
    ...
    @PatchMapping("/{iduser}") public ResponseEntity<?> partialUpdate(@PathVariable("iduser") Long iduser, @Valid @RequestBody UserUpdateRequest request, Principal principal){ if(!authId(principal).equals(iduser)) throw new UnauthorizedOperationException("Só pode editar o próprio usuário"); userService.partialUpdate(iduser, request); return ResponseEntity.ok().build(); }
    @PutMapping("/{iduser}") public ResponseEntity<?> fullUpdate(@PathVariable("iduser") Long iduser, @Valid @RequestBody UserRequest request, Principal principal){ if(!authId(principal).equals(iduser)) throw new UnauthorizedOperationException("Só pode editar o próprio usuário"); userService.fullUpdate(iduser, request); return ResponseEntity.ok().build(); }
